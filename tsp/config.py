...
class TSPConfig:
    def __init__(self):
        ...
        self.num_nodes = 100  # Number of nodes on which to train
        ...
        self.gumbeldore_config = {
            "active_search": None,
            # If a path is given here, we don't randomly generate instances but load them from here.
            "use_best_model_for_generation": True,
            # if this is True, we do not take the current model checkpoint to generate data, but the best one so far.
            "append_if_not_new_best": True,
            # if this is True, and use_best_model_for_generation is True, we do not reset the dataset after each epoch when the model has not improved, but append to the current.
            "devices_for_workers": ["cpu"] * 1,
            # List of devices where the i-th data generation worker uses the i-th device.
            "num_instances_to_generate": 1000,  # Number of instances to generate in each epoch.
            "destination_path": "./data/tsp_gumbeldore_dataset.pickle",
            # Path to save the generated dataset to in each epoch. (gets overwritten)
        ...
        self.devices_for_eval_workers = [
                                            "cpu"] * 4  # List of devices on which evaluation is performed. Here,  the i-th worker uses the i-th device.
        ...
        self.training_set_path = "./data/tsp/tsp_100_10k.pickle"
        ...
        self.custom_num_batches = ("absolute", 1000)  # See `tsp/dataset.py`
        self.validation_set_path = "./data/tsp/tsp_100_10k.pickle"
        self.validation_custom_num_instances = None  # If integer n is given, only the first n instances are used from validation set.
        self.test_set_path = "./data/tsp/tsp_100_10k_seed1234.pickle"
